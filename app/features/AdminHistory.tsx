...
export const AdminHistory = ({ client, styles }: any) => {
  ...
  const initialInput = {
    dtFrom: "", dtTo: "", uName: "", uEmail: "", src: "", dst: "", minP: "", maxP: ""
  };
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter(t => {
      ...
      return d >= from && d <= to && uNM && uEM && srcM && dstM && minM && maxM;
    });
  }, [transactions, query]);

  const downloadCSV = () => {
    const now = new Date().toISOString().split('T')[0].replace(/-/g, '');
    const headers = "ユーザー名,メールアドレス,日時,交換元,交換ポイント,交換元残高,交換先/商品名,交換先残高\n";
    const rows = filtered.map(t => 
      `${t.uName},${t.userEmail},${t.createdAt ? new Date(t.createdAt).toLocaleString() : ""},${t.rawSrc},${t.amount},${t.srcBalance},${t.rawDst},${t.dstBalance}`
    ).join("\n");
    ...
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-50 shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          {[
            { label: "開始日時 (秒まで)", key: "dtFrom", type: "datetime-local", step: "1" },
            { label: "終了日時 (秒まで)", key: "dtTo", type: "datetime-local", step: "1" },
            ...
            <div key={item.key} className="space-y-1">
              <label className="text-[11px] font-black text-black uppercase ml-1 block">{item.label}</label>
              <input type={item.type} step={item.step} className="w-full p-3 bg-slate-50 rounded-xl text-xs outline-none" 
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { setInput(initialInput); setQuery(null); }} className="px-8 py-4 bg-slate-100 text-slate-600 text-[11px] font-black rounded-2xl hover:bg-slate-200 transition-all">条件クリア</button>
          ...
          <tbody className="divide-y divide-slate-50">
            {filtered.map((t: any) => (
              <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="p-6">
                  <div className="text-sm font-black text-slate-800">{t.uName}</div>
                  <div className="text-[10px] text-slate-400 font-bold">{t.userEmail}</div>
                  <div className="text-[9px] text-slate-300 mt-1">{new Date(t.createdAt).toLocaleString('ja-JP')}</div>
                </td>
                <td className="p-6">
                  <div className="text-[11px] font-bold text-slate-600 space-y-1">
                    <div className="bg-slate-50 p-1 px-2 rounded-lg inline-block">{t.displayFrom}</div>
                    <div className="text-orange-500 font-black px-4 leading-none">↓</div>
                    <div className="bg-orange-50/50 p-1 px-2 rounded-lg inline-block border border-orange-100">{t.displayTo}</div>
                  </div>
                </td>
                <td className="p-6 text-right font-black text-orange-500 text-xl">
