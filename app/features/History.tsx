...
export const HistoryList = ({ client, userEmail, styles }: any) => {
  ...
  const downloadCSV = () => {
    ...
    const rows = filtered.map(t => 
      `${t.createdAt ? new Date(t.createdAt).toLocaleString() : ""},${t.trackingNumber},${t.rawSrc},${t.amount},${t.srcAfter},${t.rawDst},${t.dstAfter ?? ""}`
    ).join("\n");
    ...
    <div className="p-6">
      ...
      <div className="mb-6 flex justify-between items-center">
        <button 
          ...
          className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-slate-100 rounded-2xl text-[11px] font-black text-slate-600 hover:bg-slate-50 transition-all shadow-sm"
        >
          <span>{showSearch ? "✕ 検索パネルを閉じる" : "🔍 履歴を検索する"}</span>
        </button>
        {!showSearch && (
      ...
      {showSearch && (
        <div className="bg-white p-6 rounded-[2rem] mb-8 border-2 border-slate-100 shadow-sm space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <div className="flex gap-3">
            <button onClick={() => { setQuery({...input}); setShowSearch(false); }} className="flex-1 py-3 bg-slate-900 text-white text-[11px] font-black rounded-xl hover:bg-orange-500 transition-all shadow-lg">検索開始</button>
